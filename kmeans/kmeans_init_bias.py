...
def run_kmeans(X, n_clusters, n_runs, y_true, custom_init_centers=None):
    nmis = []
    aris = []
    f1s = []
    times = []
    ...
    for i in range(n_runs):
        # 设置初始化方式
        if custom_init_centers is not None:
            init_method = custom_init_centers
            n_init = 1  # 必须设为1，否则会多次随机初始化，忽略自定义中心
            random_state = None  # 自定义初始化时，random_state 对 init 无影响
        else:
            init_method = 'k-means++'
            n_init = 10
            random_state = i
        ...
        clustering = KMeans(
            ...
            init=init_method,
            n_init=n_init,
            random_state=random_state  # 控制每次运行的可重复性（除了初始化）
        )
        ...
        # 可选：对标签进行对齐（如果你希望类标签与真实标签一致）
        # 注意：align_labels 需要你自己定义或确保方向一致
        try:
            labels = align_labels(y_true, labels)
        except:
            pass  # 如果没有定义 align_labels，跳过
    ...
    return np.mean(nmis), np.mean(aris), np.mean(f1s), np.mean(times), predicted_labels[-1], nmis, aris, f1s
...
def process_single_dataset(key, mat_file, x_key, y_key, n_runs=10):
    ...
    try:
        ...
        # --- 新增：随机选择一个类，并从中随机选两个点作为初始中心 ---
        unique_labels = np.unique(y_true)
        # 随机选一个类（比如类别 0, 1, ... 中随机挑一个）
        selected_class = np.random.choice(unique_labels)
        # 找出该类的所有样本
        class_indices = np.where(y_true == selected_class)[0]
        # 从该类中随机选两个点（允许重复？不允许则 replace=False）
        chosen_indices = np.random.choice(class_indices, size=2, replace=len(class_indices) >= 2)
        custom_centers = X[chosen_indices]  # 形状: (2, n_features)
        print(f"Initialized with 2 points from class {selected_class}, indices: {chosen_indices}")

        mean_nmi, mean_ari, mean_f1, mean_times, labels, nmis, aris, f1s = run_kmeans(X, n_clusters, n_runs, y_true, custom_init_centers=custom_centers)
